...
echo "[ec2] Forwarding ports for IPFS (4001, 5001, 8081)"
sudo curl \
  --unix-socket /tmp/network.sock \
  http:/unix/services/forwarder/expose \
  -X POST \
  -d '{"local":":4001","remote":"192.168.127.2:443"}'

sudo curl \
  --unix-socket /tmp/network.sock \
  http:/unix/services/forwarder/expose \
  -X POST \
  -d '{"local":":5001","remote":"192.168.127.2:443"}'

sudo curl \
  --unix-socket /tmp/network.sock \
  http:/unix/services/forwarder/expose \
  -X POST \
  -d '{"local":":8081","remote":"192.168.127.2:443"}'
